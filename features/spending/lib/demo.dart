...
import '/spending_income_expenses_header.dart';
...
class SpendingTrackerDemo extends StatefulWidget {
  const SpendingTrackerDemo({super.key});

  static const routePath = '/spending';

  static Route<void> route(RouteSettings? settings) {
    return MaterialPageRoute(
        settings: settings, builder: (context) => const SpendingTrackerDemo());
  }

  @override
  State createState() => _SpendingTrackerDemoState();
}

class _SpendingTrackerDemoState extends State<SpendingTrackerDemo>
    with SingleTickerProviderStateMixin {
  static final _data = ChartData();
  late final Chart _chart = Chart([_data.dataSet1, _data.dataSet2], '', '\$');
